...
var SekindoClientDetections_URL = function (config) {
	...
	this.searchMetaCategory = config.searchMetaCategory;
	this.searchMetaSubCategory = config.searchMetaSubCategory;
	...
	this.setInfo = function()
	{
		...
		if (this.searchMetaCategory)
		{
			this._setMetaCategory(w.top, "name", this.searchMetaCategory, 1);
		}
		if (this.searchMetaSubCategory)
		{
			this._setMetaCategory(w.top, "name", this.searchMetaSubCategory, 2);
		}
	...
	{
		let tagContent = null;
		const metaTags = orgWin.document.getElementsByTagName("meta");
	...
	this._setMetaCategory = function(w, property, searchProperty, tagType)
	{
		const categoryContent = this._getMetaTagContent(w, property, searchProperty);
		if (categoryContent)
		{
			if (tagType == 1)
			{
				this.helperParameters.metaCategory = categoryContent;
			}
			if (tagType == 2)
			{
				this.helperParameters.metaSubCategory = categoryContent;
			}
		}
	};
	...
	this._setHelperParameters = function()
	{
		if (Object.keys(this.helperParameters).length > 0)
		{
			...
			let jsonFormattedHelperParameters = JSON.stringify(this.helperParameters);
	...
	this.setInfo();
				...
				var urlDetObj = new SekindoClientDetections_URL({
					url: 'https://live.sekindo.com/live/liveView.php?s=102181&cbuster=[CACHE_BUSTER]&pubUrl=[PAGE_URL_ENCODED]&subId=[SUBID_ENCODED]&x=[WIDTH]&y=[HEIGHT]&vp_content=plembed1993vrztgmkq&vp_template=7217&cbuster=1591193633',
					origQString: 's=102181&cbuster=[CACHE_BUSTER]&pubUrl=[PAGE_URL_ENCODED]&subId=[SUBID_ENCODED]&x=[WIDTH]&y=[HEIGHT]&vp_content=plembed1993vrztgmkq&vp_template=7217',
					placementId: 102181,
					x: 400,
					y: 225,
					videoType: 'sticky',
					...
					isAPI: false,
					debug: 0,
					ci: {"extra":{"osVersionMinor":0,"schemaVer":11,"os":"Windows","osVersionMajor":10,"deviceManufacturer":"","browser":"Chrome","browserVersionMajor":"83","deviceCodeName":"","deviceType":"desktop","deviceModel":"","osVersion":"10.0","browserType":"browser","browserVersion":"83.0.4103.61","browserVersionMinor":"0"},"browser":"chrome","os":"windows","osVer":"","deviceType":"desktop"},
					geo: 'BE',
					dmaCode: 0,
					dynamicSetup: [],
					uuid: '5ed7b021808fe',
